...
### 📝 保存与加载示例
将有效的请求参数和响应数据回写到 YAML 文件作为 OpenAPI `example`。之后可重新加载以实现可复现的测试。请求编辑器中的 **Examples 页签**以表格化列表展示已保存示例（环境 / 名称列），支持内联改名、删除、搜索与一键填充。

### 🕘 历史面板
每次发送的请求都会自动记录到表格化的**历史**列表中，支持搜索与删除。双击记录即可将完整请求（接口、认证、参数、请求头、请求体）恢复回测试表单（按 YAML 文件与所选环境匹配）。

### 📋 所有接口页签
当前 YAML 文件的 JCheckBox 接口列表，支持行号定位跳转、导出，打开文件时自动加载。
